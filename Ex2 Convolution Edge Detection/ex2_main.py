...
def MSE(a: np.ndarray, b: np.ndarray) -> float:
    print(" MSE gives error, i canceled the method")
    return np.square(a - b).mean()


def MAE(a: np.ndarray, b: np.ndarray) -> float:

    print(" MAE gives error, i canceled the method")
...
def houghDemo():
    ...
    cv2_cir = cv2.HoughCircles((img * 255).astype(np.uint8), cv2.HOUGH_GRADIENT, 1, 2, param1=50,
